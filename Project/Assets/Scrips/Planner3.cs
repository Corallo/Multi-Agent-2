...
public class Planner3 {
	...
	public void ComputePath(TerrainManager _terrain_manager) {
		mask = LayerMask.GetMask("CubeWalls");
		...
		Vector3 start_pos = friends[0].transform.position;
		...
		car_targets = OldTabuSearch.RunTabuSearch(3, point_of_interest, CostMatrix, PathMatrix, 100, 2000, 1000);
		...
		foreach (var car in friends) {
			...
			if (j == 0) {
				...
				car.GetComponent<CarAI3>().skipper = 150;
			} else {
				car.GetComponent<CarAI3>().color = "Blue";
				car.GetComponent<CarAI3>().skipper = 250;
			}
			j++;
		}
	...
	public void RRG(int max_nodes, Graph G) {
		...
		for (int i = 0; i < max_nodes && max_iter > 0; i++) {
			...
			for (max_iter = 10000; !found && max_iter > 0; max_iter--) {
				...
				if (distance > edgeLength) {  //skip if B too close 
