...
public class Planner5Special {
    ...
    public void ComputePath(TerrainManager _terrain_manager) {
        ...
        foreach (var wp in best_path) {
            ...
            Debug.DrawLine(old_wp, DroneGraph.getNode(wp).getPosition(), colors[coloridx], 100f);
        ...
        */
        ...
        friends[1].GetComponent<CarAI5>().skipper = 100;
        friends[2].GetComponent<CarAI5>().skipper = 150;
    ...
    Graph generateGraph(Graph G) {
        ...
        for (int i = (int)terrain_manager.myInfo.x_low+10; i < (int)terrain_manager.myInfo.x_high; i+=density)
        {
            ...
            {
                if (terrain_manager.myInfo.traversability[terrain_manager.myInfo.get_i_index(i), terrain_manager.myInfo.get_i_index(j)] == 0f) {
