...
public class Planner5 {
    ...
    public void ComputePath(TerrainManager _terrain_manager) {
        ...
        /*
        foreach (var checkpoint in point_of_interest) {
        ...
        float pathConstant = 0.2f;
        int dest = 0;
        float destinationConstant = 1f;
        ...
        for (int k = 0; k < point_of_interest.Count; k++) {
            ASuperStar(DroneGraph, startNode, point_of_interest[k]);
            List<int> tmp_path = getBestPath(DroneGraph, startNode, point_of_interest[k]);
            PathMatrix[new Tuple<int, int>(startNode, point_of_interest[k])] = new List<int>(tmp_path); // ADD To the real path, the path to get the ith point 
            CostMatrix[new Tuple<int, int>(startNode, point_of_interest[k])] = (computePathCost(DroneGraph, tmp_path));
            totalCost = (pathConstant * CostMatrix[new Tuple<int, int>(startNode, point_of_interest[k])] + destinationConstant * DroneGraph.getNode(point_of_interest[k]).getDanger());
            if (BestCost > totalCost && totalCost != 0 && tmp_path.Count > 1 && DroneGraph.getNode(tmp_path[tmp_path.Count - 1]).getDanger() > 0) {
                BestCost = totalCost;
                dest = k;
            }
        ...
        */
        ...
        /*SUM COST OF A STAR */
        best_path = PathMatrix[new Tuple<int, int>(startNode, dest)];
        ...
        foreach (var wp in best_path) {
            ...
            Debug.DrawLine(old_wp, DroneGraph.getNode(wp).getPosition(), Color.cyan, 25f);
        ...
        my_path = pathHelp(DroneGraph, best_path);
        foreach (var car in friends) {
            ...
            car.GetComponent<CarAI5>().lastPointInPath = -1;
        ...
        for (int i = st; i < st + friends.Length; i++) {
            if (i == st) {
                ...
                friends[i % friends.Length].GetComponent<CarAI5>().leaderOffset = 8f;

            }

            if (i == st + 1) {
                friends[i % friends.Length].GetComponent<CarAI5>().offset = 6f;
                friends[i % friends.Length].GetComponent<CarAI5>().leaderOffset = -4f;
            }
